...
// Current returns the active configuration. It is safe for concurrent
// use.
...
// Subscribe returns a channel that receives each accepted
// configuration. The channel has capacity 1 and drops intermediate
// values, because only the newest configuration matters.
...
// rejected and the previous value stays active, so a bad edit cannot
// break a running exporter.
...
func (w *Watcher) reload(force bool) {
	...
	if cfg.NamingChanged(prev) {
		slog.Warn("naming configuration changed; every group name is rebuilt " +
			"and every time series will break")
	} else {
